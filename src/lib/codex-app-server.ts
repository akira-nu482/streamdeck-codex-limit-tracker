...
type CurrentRateLimitWindow = {
  usedPercent: number;
  windowDurationMins: number;
  resetsAt: number;
};

type CurrentRateLimit = {
  limitId: string;
  limitName?: string | null;
  primary: CurrentRateLimitWindow;
  secondary: CurrentRateLimitWindow;
  planType?: string;
};
...
type AppServerRateLimitResult =
  | RawUsageBody
  | {
      rateLimits?: CurrentRateLimit;
      rateLimitsByLimitId?: Record<string, CurrentRateLimit>;
    }
  | {
      email?: string;
      planType?: string;
...
function normalizeWindowCurrent(window: CurrentRateLimitWindow, label: string): DisplayWindow {
